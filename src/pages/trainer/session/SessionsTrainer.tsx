...
export default function SessionsTrainer() {
  ...
  const handleSaveEdit = async (values: UpdateSessionPayload) => {
    ...
    try {
      ...
      setSnack({ open: true, message: "Sessão atualizada com sucesso.", severity: "success" });
      setEditOpen(false);
      setEditId(null);
      setEditInitial(null);
    } catch (e: any) {
      setSnack({
        open: true,
        message: e?.response?.data?.message ?? "Falha ao atualizar sessão.",
        severity: "error",
      });
    } finally {
      setEditSaving(false);
      ...
      <Box sx={{ px: 8, pt: 4, pb: 8 }}>
        ...
        >
          ...
          >
            ...
            >
              <AddIcon onClick={() => navigate("/trainer-new-session")} />
            </IconButton>
          </Box>
        </Paper>
